...
class _RushGameScreenState extends ConsumerState<RushGameScreen> {
  ...
  /// Maps [RushPhase] to the [GamePhase] enum used by reusable widgets.
  GamePhase _toGamePhase(RushPhase phase) {
    switch (phase) {
      case RushPhase.loading:
        return GamePhase.loading;
      case RushPhase.answering:
        return GamePhase.answering;
      case RushPhase.showingFeedback:
  ...
  Widget _buildBody(BuildContext context, RushState state) {
    ...
    final timerColor = state.timeRemainingMs > 10000
        ? AppColors.primary
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  '${(state.timeRemainingMs / 1000).ceil()}s',
                  style: AppTypography.labelSmall.copyWith(
                    color: timerColor,
                    letterSpacing: 1,
                  ),
